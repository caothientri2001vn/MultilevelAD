...
def evaluation_ours_mvtec(encoder, bn, decoder, dataloader,device):
    ...
    paths = []
    with torch.no_grad():
        for (img, gt, label, _, path) in tqdm(dataloader):
            path = path[0]
            img = img.to(device)
            inputs = encoder(img)
...
def evaluation_ours_class(encoder,bn, decoder, dataloader,device):
    encoder.eval()
    bn.eval()
    decoder.eval()
    # gt_list_px = []
