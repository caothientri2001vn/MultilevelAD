...
class Skipganomaly(BaseModel):
    ...
    def backward_d(self):
        ...
        # Combine losses.
        err_g_lat = self.opt.w_lat * self.l_lat(feat_fake, self.feat_real)
        self.err_d = self.err_d_real + self.err_d_fake + err_g_lat
    ...
    def test_to_file(self):
        ...
        with torch.no_grad():
            # Load the weights of netg and netd.
            if self.opt.load_weights:
                self.load_weights(is_latest=True)
            ...
            # Create big error tensor for the test set.
            self.an_scores = torch.zeros(size=(len(self.data.test.dataset),), dtype=torch.float32, device=self.device)
            self.files = []
            self.features  = torch.zeros(size=(len(self.data.test.dataset), self.opt.nz), dtype=torch.float32, device=self.device)

            print("   Testing %s" % self.name)
            self.times = []
            self.total_steps = 0
            epoch_iter = 0
            for i, data in tqdm(enumerate(self.data.test, 0), total=len(self.data.test)):
                self.total_steps += self.opt.batchsize
                epoch_iter += self.opt.batchsize
                time_i = time.time()

                # Forward - Pass
                self.set_input_test(data)
                self.fake = self.netg(self.input)
                ...
                self.an_scores[i*self.opt.batchsize: i*self.opt.batchsize + error.size(0)] = error.reshape(error.size(0))
                self.files.extend(self.file_path)
