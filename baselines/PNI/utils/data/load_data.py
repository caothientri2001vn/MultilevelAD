...
class MVTecDataset(Dataset):
    ...
    def load_dataset(self):
        ...
        for defect_type in defect_types:
            if defect_type == 'good':
                img_paths = glob.glob(os.path.join(self.img_path, defect_type) + "/*.[jJ][pP][gG]") + glob.glob(os.path.join(self.img_path, defect_type) + "/*.[jJ][pP][eE][gG]") + glob.glob(os.path.join(self.img_path, defect_type) + "/*.[pP][nN][gG]")
                img_paths.sort()
...
class CustomMVTecDataset(Dataset):
    def __init__(self, args, transform, gt_transform):
        self.opt = args
    ...
    def __getitem__(self, idx):
        img = Image.open(os.path.join(self.opt.test_dir, self.files[idx])).convert('RGB')
        img = self.transform(img)
...
def Custom_Test_Dataloader(args):
    data_transforms = Transform(args.resize, args.imagesize)
    gt_transforms = GT_Transform(args.resize, args.imagesize)

    test_datasets = CustomMVTecDataset(args, transform=data_transforms, gt_transform=gt_transforms)
    test_loader = DataLoader(test_datasets, batch_size=1, shuffle=False, num_workers=args.num_workers, pin_memory=True)
    return test_loader
