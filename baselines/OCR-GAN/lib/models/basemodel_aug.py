...
class BaseModel_Aug():
    ...
    def set_input(self, input:torch.Tensor, noise:bool=False):
        ...
        ##
    def set_input_test(self, input:torch.Tensor, noise:bool=False):
        """ Set input
    ...
    ##
    def save_weights(self, epoch:int, is_best:bool=False, is_latest:bool=False):
        ...
        if is_best:
            ...
            torch.save({'epoch': epoch, 'state_dict': self.netg.state_dict()}, f'{weight_dir}/netG_latest.pth')
            torch.save({'epoch': epoch, 'state_dict': self.netd.state_dict()}, f'{weight_dir}/netD_latest.pth')
        else:
    ...
    def load_weights(self, epoch=None, is_best:bool=False, is_latest=False, path=None):
        ...
        if epoch is None and is_best is False and is_latest is False:
        ...
        if is_best:
            ...
            fname_g = f"netG_latest.pth"
            fname_d = f"netD_latest.pth"
        else:
    ...
    def train_one_epoch(self):
        ...
        for data in tqdm(self.data.train, leave=False, total=len(self.data.train)):
            ...
            err_g += self.err_g.item() * data[0].size(0)
            err_d += self.err_d.item() * data[0].size(0)
        ...
        err_g /= len(self.data.train.dataset)
        err_d /= len(self.data.train.dataset)

        print(f"{err_d = :.4f} | {err_g = :.4f}")

    def save_scores(self):
        print(self.an_scores)
        with open(self.opt.out_file, 'w') as f:
            f.write(f'Path,Anomaly Score\n')
            for an_score, file_path in zip(self.an_scores, self.files):
    ...
    def train(self):
        ...
        for self.epoch in range(self.opt.iter, self.opt.niter):
            ...
            if res['AUC'] > best_auc:
                ...
                self.save_scores()
            self.save_weights(self.epoch, is_latest=True)
