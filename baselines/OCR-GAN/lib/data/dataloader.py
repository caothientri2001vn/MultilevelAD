...
from lib.data.datasets import ImageFolder_FD, ImageFolder, ImageFolder_FD_Aug, FD
class Cutout(object):
...
class FileListDataset(Dataset):
    def __init__(self, opt, transform=None, transform_aug=None):
        super().__init__()
        self.opt = opt
    ...
    def __getitem__(self, idx):
        img = cv2.imread(os.path.join(self.opt.testroot, self.files[idx]))
        lap, res = FD(img)
        ...
        target = 0 if 'level_0' in self.files[idx] else 1
        return lap, res, fake_aug, target, self.files[idx]
...
def load_data_FD_aug(opt, classes):
    ...
    valid_ds = FileListDataset(opt, transform, transform_aug)
