...
def load_test_our(test_path, severities, sample_rate):
    transform = torchvision.transforms.Compose([
        torchvision.transforms.Resize(256),
        torchvision.transforms.CenterCrop(256),
        torchvision.transforms.ToTensor(),
    ...
    imagenet_data = MvtecDataLoader_our(path=test_path, severities=severities, transform=transform, mode="test", sample_rate=sample_rate)
    print("imagenet_data", len(imagenet_data))
...
def load_test(test_path, severities, sample_rate):
    transform = torchvision.transforms.Compose([
        torchvision.transforms.Resize(256),
        torchvision.transforms.CenterCrop(256),
        torchvision.transforms.ToTensor(),
...
def train(args, NORMAL_NUM,
    ...
    train_path = '{}/../../data/Industry/mvtec/{}/train/good'.format(args.work_dir, NORMAL_NUM)
    START_ITER = 0
    ...
    for iteration in process:
        ...
        if iteration == END_ITER - 1:
            ...
            df = pd.DataFrame({
                ...
                'Severity': severities,
                'Anomaly Score': score,
                # 'Anomaly Score Sum': pr_list_sum
            })


                # Lưu DataFrame vào file CSV
            df.to_csv('{}/result_csvs/IGD_mvtec_{}.csv'.format(args.work_dir, NORMAL_NUM), index=False)
...
def validation(NORMAL_NUM, iteration, generator, discriminator, real_data, fake_data, is_end, AUC_LIST, END_ITER, args = None):
    ...
    image_paths = []
    if args is not None:
        test_root = '{}/../../data/Industry/mvtec/{}/test/'.format(args.work_dir, NORMAL_NUM)
        path_template = '{}/../../data/template/mvtec_{}_template.csv'.format(args.work_dir, NORMAL_NUM)
    else:
        test_root = '/home/tri/multi-level-anomaly/data/corrupted/brightness/mvtec_order/{}/test/'.format(NORMAL_NUM)
        path_template = '/home/tri/multi-level-anomaly/data/template/mvtec_{}_template.csv'.format(NORMAL_NUM)
    ...
    img_tot_paths = [test_root + item for item in img_tot_paths]
    tot_types = columns_as_lists['Severity']

    with torch.no_grad():
        valid_dataset_loader = load_test_our(img_tot_paths, tot_types, sample_rate=1.)

        for index, (image_path, images, label, severity) in enumerate(valid_dataset_loader):
            img = images.to(device)
            # print(img.shape)
            severities.append(severity.detach().cpu().item())
            ...
            ############################## Normal #####################
            # print("latent_z.shape[0]", latent_z.shape[0])
    ...
    image_paths = [item[0].replace(test_root, '') for item in image_paths]
    return auc_result, AUC_LIST, image_paths, severities, score
...
if __name__ == "__main__":
    ...
    for key in sorted(sub_category):
        ...
        # train_path = '/home/tri/multi-level-anomaly/data/area-based/mvtec/{}/train/'.format(NORMAL_NUM)
        # test_root = '/home/tri/multi-level-anomaly/data/area-based/mvtec/{}/test/'.format(NORMAL_NUM)
        # gt_root = '/home/tri/multi-level-anomaly/data/area-based/mvtec/{}/ground_truth/'.format(NORMAL_NUM)
