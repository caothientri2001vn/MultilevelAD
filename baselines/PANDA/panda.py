...
def train_model(model, train_loader, test_loader, device, args, ewc_loss):
    ...
    for epoch in range(args.epochs):
        ...
        auc, feature_space = get_score(model, device, train_loader, test_loader)
        torch.save(model.state_dict(), os.path.join(args.output_dir, args.dataset, 'latest.pth'))
        if auc > max_auc:
            torch.save(model.state_dict(), os.path.join(args.output_dir, args.dataset, 'best.pth'))
            max_auc = auc
...
def get_score(model, device, train_loader, test_loader):
    ...
    with torch.no_grad():
        test_labels = []
        for (imgs, targets) in tqdm(test_loader, desc='Test set feature extracting'):
            imgs = imgs.to(device)
            _, features = model(imgs)
...
def main(args):
    ...
    train_loader, test_loader = utils.get_loaders(dataset_dir=args.dataset_dir, batch_size=args.batch_size)
