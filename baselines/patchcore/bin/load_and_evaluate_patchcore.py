...
_DATASETS = {"mvtec": ["patchcore.datasets.mvtec", "MVTecDataset"],
             "visa": ["patchcore.datasets.mvtec", "MVTecDataset"],
             "example": ["patchcore.datasets.mvtec", "MVTecDataset"],
             "diabetic_retinopathy": ["patchcore.datasets.mvtec", "MVTecDataset"],
             "covid19": ["patchcore.datasets.mvtec", "MVTecDataset"],
             "skin_lesion": ["patchcore.datasets.mvtec", "MVTecDataset"]}


@click.group(chain=True)
@click.argument("results_path", type=str)
@click.option("--dataset", type=str, default='mvtec')
...
def run(methods, results_path, gpu, seed, save_segmentation_images, dataset):
    ...
    # print('n_dataloaders', n_dataloaders) # 15
    # print('n_patchcores', n_patchcores) # 15
    ...
    for dataloader_count, dataloaders in enumerate(dataloader_iter):
        ...
        # print('dataloaders', len(dataloaders)) # 1
        # print('dataset_name', dataloaders["testing"].name) # mvtec_bottle, ...
        # print('dataloader_count', dataloader_count) # 0, 1, 2, ..., 14
        ...
        with device_context:
            ...
            for i, PatchCore in enumerate(PatchCore_list):
                ...
                print(f'scores {dataloader_count}: ', len(scores))
...
@click.option("--ood_noise_type", type=str, default='')
def patch_core_loader(patch_core_paths, faiss_on_gpu, faiss_num_workers, ood_noise_type):
    def get_patchcore_iter(device):
        for patch_core_path in patch_core_paths:
            ...
            if n_patchcores == 1:
                ...
                patchcore_instance = patchcore.patchcore.PatchCore(device, ood_noise_type)
...
@click.option("--ood_noise_type", type=str, default='')
def dataset(
    name, data_path, subdatasets, batch_size, resize, imagesize, num_workers, augment, ood, ood_noise_type
):
    # print('name', name) # mvtec
    # print('data_path', data_path) # /home/tri/multi-level-anomaly/data/area-based/mvtec
    # print('subdatasets', subdatasets) # ('bottle', 'cable', 'capsule', 'carpet', 'grid', 'hazelnut', 'leather', 'metal_nut', 'pill', 'screw', 'tile', 'toothbrush', 'transistor', 'wood', 'zipper')
