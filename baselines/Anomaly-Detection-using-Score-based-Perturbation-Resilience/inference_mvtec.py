...
def inference(class_name, path_test_data, root):
    ...
    load_pth = f'/home/tri/llms-anomaly-detection/new/Anomaly-Detection-using-Score-based-Perturbation-Resilience/Pretrained_weight_mvtec/{class_name}.pth'
    ...
    for x, y, path, level in test_loader:
        path_ = path[0]
    ...
    print(paths)
    scores = heatmaps.max(axis = (1,2,3))
    max_score = scores.reshape(scores.shape[0], -1).max(axis=1)
    print(class_name, max_score)
    
    # for i in range(heatmaps.shape[0]):
    #     sample = heatmaps[i]
    #     # sample_sum = np.sum(sample)
    #     # sample_max = np.max(sample)
    #     # sample_sum = heatmap.sum(axis=(1,2,3va)
...
if __name__ == '__main__':
    ...
    subsets_mvtec = ['bottle']
    root_general = '/home/tri/multi-level-anomaly/data/area-based/mvtec_order/'
    for subset in subsets_mvtec:
        ...
        # df.to_csv('/home/tri/llms-anomaly-detection/new/Anomaly-Detection-using-Score-based-Perturbation-Resilience/results/score-based' + 'mvtec' + '_' + subset +'.csv', index=False)
