...
def calc_prob_embedding(distances, gamma):
    prob_embedding = gamma * torch.exp(-gamma*distances)
...
class AC_Model(pl.LightningModule):
    def __init__(self, args, dist_input_size, dist_output_size):
        ...
        self.coor_dist_model = torch.from_numpy(np.load(os.path.join(self.embedding_dir_path, f'coor_model_sp{int(self.args.subsampling_percentage*100)}.npy')))
        if torch.cuda.is_available():
            self.coor_dist_model = self.coor_dist_model.cuda()
    ...
    def calc_anomaly_pxl(self, dist_matrix, epsilon=1e-6) :
        ...
        '''
        dist_matrix_sorted, _ = torch.sort(-dist_matrix, dim = 1)

        return -torch.log(-dist_matrix_sorted[:, 0])
    ...
    def on_test_start(self):
        ...
        self.emb_to_dist = self.emb_to_dist
    ...
    def test_epoch_end(self, outputs):
        ...
        values = {'img_auc_nb': img_auc_nb, 'img_auc_coor': img_auc_coor, 'img_auc_patchcore': img_auc_patchcore, \
...
class Custom_AC_Model(AC_Model):
    def __init__(self, args, dist_input_size, dist_output_size):
        super().__init__(args, dist_input_size, dist_output_size)

    def test_step(self, batch, batch_idx): # Nearest Neighbour Search
        x, file_name = batch
        features = self(x)
        features, ref_num_patches = generate_embedding_features(self.args, features, self.patch_maker)
        embedding_test = features # (W x H) x E
        embedding_test_cpu = embedding_test.cpu() # (W x H) x E

        W, H = ref_num_patches

        pad_width = (0, 0, self.args.dist_padding, self.args.dist_padding, self.args.dist_padding, self.args.dist_padding)
        embedding_pad = torch.nn.functional.pad(embedding_test.view(W, H, -1), pad_width, "constant").cpu().numpy() # (W+1) x (H+1) x E
        ...
        neighbors = torch.from_numpy(neighbors).to(embedding_test.device)

        ## evaluate anomaly map from patchcore
        embedding_score, _ = self.embedding_coreset_with_edge_index.search(embedding_test, k=self.args.anomaly_nn) # (W x H) x self.args.n_neighbors
        embedding_score = torch.sqrt(embedding_score)

        max_anomaly_idx = torch.argmax(embedding_score[:, 0])
        max_embedding_score = embedding_score[max_anomaly_idx, 0] # maximum embedding score
        if self.args.anomaly_nn == 1 :
            weights_from_code = 1
        else :
            weights_from_code = 1 - torch.exp(max_embedding_score) / torch.sum(torch.exp(embedding_score[max_anomaly_idx]))

        anomaly_img_score_patchcore = weights_from_code * max_embedding_score # Image-level score

        anomaly_map_patchcore = embedding_score[:, 0].view(ref_num_patches) # Pixel-level score

        ## evaluate anomaly map using position and neighborhood information
        anomaly_img_score_nb_coor = anomaly_img_score_patchcore
        anomaly_map_nb_coor = anomaly_map_patchcore

        # trained normal feature distirbution using neighborhood information
        neighbors = neighbors.view(-1, neighbors.shape[2]).to(torch.float32) # (W x H) x NE
        y_hat = self.dist_model(neighbors) # (W x H) x self.dist_coreset_index.ntotal
        softmax_nb_temp = F.softmax(y_hat / self.args.softmax_temperature_alpha, dim = -1) # (W x H) x self.dist_coreset_indesx.ntotal
        softmax_nb_thres = softmax_nb_temp  > self.args.softmax_nb_gamma / self.embedding_coreset_index.ntotal # threshold of softmax
        # #(revise 23/03/10) change self.embedding_coreset_index.ntotal to self.dist_coreset_index.ntotal
        # softmax_nb_thres = softmax_nb_temp  > self.args.softmax_nb_gamma / self.dist_coreset_index.ntotal # threshold of softmax
        ...
        softmax_coor = self.coor_dist_model
        softmax_coor_thres = softmax_coor > self.args.softmax_coor_gamma / self.embedding_coreset_index.ntotal # threshold of softmax
        embed_distances, embed_indices = self.embedding_coreset_index_cpu.search(embedding_test_cpu, k=self.embedding_coreset_index.ntotal) # (W x H) x self.dist_coreset_index.ntotal
        embed_indices = embed_indices.to(embedding_test.device)
        embed_distances = torch.sqrt(embed_distances).to(embedding_test.device)
        embed_prob = calc_prob_embedding(embed_distances, gamma=self.args.prob_gamma)
        ...
        # softmax_coor_with_edge = self.coor_dist_model_with_edge
        # softmax_coor_thres_with_edge = softmax_coor_with_edge > self.args.softmax_coor_gamma / self.embedding_coreset_with_edge_index.ntotal # threshold of softmax
        # embed_distances_with_edge, embed_indices_with_edge = self.embedding_coreset_with_edge_index_cpu.search(embedding_test_cpu, k=self.embedding_coreset_with_edge_index.ntotal) # (W x H) x self.dist_coreset_index.ntotal
        # embed_indices_with_edge = embed_indices_with_edge
        # embed_distances_with_edge = torch.sqrt(embed_distances_with_edge)
        # embed_prob_with_edge = calc_prob_embedding(embed_distances_with_edge, gamma=self.args.prob_gamma)

        _, dist_indices = self.dist_coreset_index.search(embedding_test, k=self.dist_coreset_index.ntotal) # (W x H) x self.dist_coreset_index.ntotal

        softmax_nb_thres_inverse = torch.zeros(size = (neighbors.shape[0], self.embedding_coreset_index.ntotal), dtype=bool, device=embedding_test.device)

        # with pymp.Parallel(8) as p :
        # for j in range(softmax_nb_thres.shape[1]):
        #     idx = torch.where(self.emb_to_dist[:,0]==j)
        ...
        second_dim_masks = self.emb_to_dist == torch.arange(softmax_nb_thres.shape[1], device=embedding_test.device)
        first_dim_range = torch.arange(softmax_nb_thres.shape[0], device=embedding_test.device)

        for j in range(softmax_nb_thres.shape[1]):
            first_dim_mask = softmax_nb_thres[first_dim_range, dist_indices[:, j]]
            second_dim_mask = second_dim_masks[:, j]
            softmax_nb_thres_inverse += first_dim_mask.unsqueeze(-1) & second_dim_mask

        softmax_nb_thres_inverse[:, -1] = True


        # softmax_coor_thres_inverse = torch.zeros(size = (neighbors.shape[0], self.embedding_coreset_index.ntotal), dtype=bool, device=embedding_test.device)
        # with pymp.Parallel(8) as p :
        ...
        # for i in range(softmax_coor_thres.shape[0]):
        #     softmax_coor_thres_inverse[i, :] = softmax_coor_thres[i, embed_indices[i, :]]

        softmax_coor_thres_inverse = softmax_coor_thres[torch.arange(softmax_coor_thres.shape[0], device=embedding_test.device).unsqueeze(-1), embed_indices]
        softmax_coor_thres_inverse[:, -1] = True
        ...
        # anomaly map using both neighborhood and position information
        anomaly_map_nb_coor = anomaly_pxl_nb_coor.view(ref_num_patches)
        anomaly_map_nb_coor = anomaly_map_nb_coor[patch_padding:anomaly_map_nb_coor.shape[0]-patch_padding, patch_padding:anomaly_map_nb_coor.shape[1]-patch_padding]
        anomaly_img_score_nb_coor = torch.max(anomaly_map_nb_coor).cpu().numpy()
        # anomaly_map_nb_coor = np.pad(anomaly_map_nb_coor, pad_width, 'edge')
    ...
    def test_epoch_end(self, outputs):
        ...
        with open(self.args.out_file, 'w') as f:
            f.write(f'Path,Anomaly Score\n')
            for an_score, file_path in zip(self.pred_list_img_lvl_nb_coor, self.img_path_list):
                f.write(f'"{file_path}",{an_score}\n')