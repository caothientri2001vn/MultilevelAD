...
class AnomalyDetector:
    ...
    def make_context(self, subset):
        if "_" in subset:
            subset = subset.replace("_"," ")
        elif "pcb" in subset:
            subset = "printed circuit board (PCB)"
        elif "macaroni" in subset:
            subset = "macaroni" 
        return subset
        
    def inference(self, image_path_test, normal_image_paths, subset, model_name):
        ...
        context = self.make_context(subset)
        # print(context)
        prompt = f"""
You are given four industrial inspection images:

Image 1, 2, 3: Belong to the non-defect category of {context}.
...
Please ensure the output strictly follows the format. You must answer the question even if you are not entirely certain about the answer. Do not answer that you cannot."""
...
for dataset_name in datasets:
    root_general = f'../data/Industry/{dataset_name}/'
    subsets = subset_dict[dataset_name]
        
        
    for subset_name in subsets:
        path_test_data = f'../data/template/{dataset_name}_{subset_name}_template.csv' 
        root = root_general + subset_name + '/test/'
        normal_image_files = glob.glob(os.path.join(root_general + subset_name + '/train/good/', "*.*"))
